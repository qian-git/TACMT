function [ node_struct ] = Copy_of_level4_func( node_struct,input_array,PX_flag,px_merge,model_upper ) %input_array = [level2_node_id,position[4],direction] 6_bits
model_cnt_initial = 1;
...
for i = 1:1:level_number
    ...
    if(node_struct{i}.work_state == 1 && ~isempty(node_struct{i}.receive))
        ...
        for j = 1:1:size_receive
%             PX = 0;
            if(node_struct{i}.receive{j}.valid_in == 1) %进行匹配
...
%                         node_choose(1,m) = size_LCS_1/size_axis;
                ...
                if(max_PX >= PX_flag)%存在合适模型
                    ...
                    node_struct{pointer_PX}.work_state = 1;
                    if(node_struct{i}.receive{j}.series_end == 1 && max_PX >= px_merge)
                        [~,size_axis] = size(node_struct{i}.receive{j}.series);
%                         for k = 1:1:size_axis
